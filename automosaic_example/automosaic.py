...
def process_single_image(image_file: str, model_paths: list[str], args) -> None:
    ...
    # Per-image auto mosaic size
    if args.auto_size:
        img_w, img_h = original_image.size
        long_side = max(img_w, img_h)
        args_mosaic_size = max(4, round(long_side / 100)) if long_side >= 400 else 4
        print(f"  - [モザイク自動サイズ] {args_mosaic_size}px (長辺: {long_side}px)")
    else:
        args_mosaic_size = args.mosaic_size
