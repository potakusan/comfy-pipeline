...
-bo, --box-only
	検出形状(マスク)を無視し、bboxの矩形全体にモザイクをかける
	斜めの物体などではマスク形状に合わせた方が余分な範囲にモザイクがかからず正確
	デフォルト オフ（マスク形状に沿ってモザイクをかける）

-rm, --retina_masks
	高解像度セグメンテーションマスクを使用する
...
＜更新履歴＞
2026/07/06 Ver.0.8
	デフォルトで検出マスク形状に沿ってモザイクをかけるよう変更（斜めの物体で余分な範囲にモザイクがかかる問題を修正）
	-umオプションを廃止し、矩形全体にモザイクをかける-boオプションを追加（挙動が反転）

2025/06/24 Ver.0.7.2
	高解像度セグメンテーションマスクを使用しない場合にマスク画像アスペクト比がおかしくなる問題が再発していたのを修正

2025/06/24 Ver.0.7.1
	ultralyticsを更新(8.2.78 -> 8.3.158)
	高解像度セグメンテーションマスクを使用するオプション-rmを追加
	デフォルトでは高解像度セグメンテーションマスクを使用しないよう変更

2025/06/24 Ver.0.7
	高解像度セグメンテーションマスクを使用するよう変更
