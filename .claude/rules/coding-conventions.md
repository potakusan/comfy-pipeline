...
- UI部品はshadcn/ui (`components/ui/`)を優先利用
- 自前の`/api/*`ルートを呼ぶときは`lib/api-client.ts`の`apiFetch<T>()`を使う。`res.ok`判定とエラーボディのパースを内包し、失敗時は`Error`をthrowする。外部サービス(ComfyUI/Civitai/LoRA Manager等)の呼び出しやJSON以外のレスポンス(画像blob等)、UIに影響しないfire-and-forget呼び出し(中断シグナル送信等)には使わず素の`fetch()`のままでよい
